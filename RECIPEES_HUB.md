...
>
> **2026-06-06 (sessione Memoria)** — avviata implementazione **acquisizione motore unico** (`SPEC_acquisizione.md`) su branch `feat/acquisizione-motore-unico` (repo Heirloom, pushato). Completato e reviewato l'intero **upgrade del motore/editor**: `Book.mode` per-libro migration-safe, `RecipeRegion` cross-foglio (additivo, non persistito), slider Sx/Dx sempre attivi (rimosso `twoColumnsMode`), **default progressivi §5.1**, crop foto sugli stessi slider, seed AI on-demand, editor su sequenza-fogli + `mode` con init compat (Doppia byte-identica). 8 commit, build verde a ogni step, spec-review indipendenti sui task chiave. **Differita la convergenza** (instradare le 3 modalità dall'acquisizione verso l'editor unico, ritiro Block*, multipagina cross-foglio): tocca 3 flussi esistenti → decisioni di prodotto + verifica a vista col fondatore, post-lancio. Checklist verifica device nel piano. Dettaglio: sez. Memoria + piano `docs/superpowers/plans/2026-06-05-acquisizione-motore-unico.md`.
>
...
- **[GESTITO 2026-06-06 da Memoria — in corso] HAND-OFF 2026-06-05 · Recipees → Memoria: SPEC acquisizione motore unico.** Preso in carico: piano scritto (`docs/superpowers/plans/2026-06-05-acquisizione-motore-unico.md`) + implementato l'intero upgrade motore/editor su branch `feat/acquisizione-motore-unico` (8 commit, build verde, review). Differita la *convergenza* (3 modalità dall'acquisizione → editor unico, ritiro Block*, multipagina cross-foglio): richiede decisioni di prodotto + verifica a vista col fondatore, post-lancio. Resta da: cattura multi-foglio, cablaggio entry-point modalità (G1/F2), ritiro Block* (H1), F1b. Brief originale qui sotto.

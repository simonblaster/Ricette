...
libro stampato, da set 2026). A lato, uno strumento "storico" ancora vivo: il **sito
ricettario** (Paprika → web).
...
condivisibile e con voce/video → un domani diventa Folio. A lato, il binario storico: il
sito legacy si aggiorna da Paprika con `aggiorna_sito.py`. Gli output non sono magie
separate: sono la conseguenza di ciò che digitalizzi all'inizio — per questo la qualità
dell'acquisizione (Memoria) determina tutto il resto.

# 🔧 Vista creator

## 1. In una frase
Tre prodotti (app iOS, web app, libro) + una pipeline legacy (sito statico da Paprika), coordinati da una sessione-supervisore via un hub condiviso e un contratto di export stabile Memoria→Domus.
...
la sua CASELLA BRIEF. A lato, il **sito legacy**: `aggiorna_sito.py` traduce il DB Paprika
in JS statico dentro `docs/` (servito da GitHub Pages).
...
Memoria `books.json` su device · sito legacy: `docs/recipes.js`+`docs/photo-uids.js`, `categorie_tracciate.json`, DB Paprika (`MacGourmet.mgdb`).
...
- **Sito legacy**: live e stabile (binario storico, manutenzione on-demand).
...
- ✅ DB Paprika (`MacGourmet.mgdb`) presente per il sito legacy.

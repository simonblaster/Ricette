...
| Fix: tag/categorie sparsi sulla home mobile | Nella home mobile (/recipes) appaiono i chip tag/categorie — duplicati rispetto alla sidebar desktop, inutili e confusivi. Vanno rimossi: i tag si filtrano solo dalla pagina /search. | Fatto | Domus · commit `dc64864`. Rimossi il blocco `CategoryFilters` e le funzioni orfane `toggleCategory`, `clearCategories`, `categoryStats` dalla `recipes/page.tsx`. Il filtro tag continua a funzionare via sidebar su desktop (AppSidebar gestisce propri link) e via /search. |

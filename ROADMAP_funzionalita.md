...
|---|---|---|---|
| Classificazione automatica delle ricette | All'acquisizione/strutturazione l'AI **classifica la ricetta** assegnandola alle **categorie già esistenti** dell'utente (vocabolario controllato, non ne inventa di nuove) ed eventualmente in base ad **altri criteri** (tipo di portata, stagionalità, tecnica, tempo di preparazione, dieta…). L'utente resta arbitro: può correggere/riassegnare. Riduce il lavoro manuale di catalogazione e tiene il ricettario ordinato fin dall'inizio. | Idea | Memoria · post-lancio. **Cross-prodotto** e strettamente legata a «Categorie vs Tag» (area Domus): le categorie sono definite/controllate lato Domus, l'AI di Memoria (e dell'import nativo Domus) deve pescare da quella lista, non crearne. Da chiarire: criteri attivabili (solo categorie vs anche stagionalità/portata/dieta), e se la classificazione è suggerita-con-conferma o automatica. Mini-spec condivisa con «Categorie vs Tag» prima dello sviluppo. |
...
| Acquisizione — motore unico, 3 modalità (Singola / Doppia / Multipagina) | Un **unico delimitatore** (`PageMultiRecipeEditor` generalizzato) per tutte le modalità: cambia solo la foto di origine. Singola (1 pagina, no spine), Doppia (spread), Multipagina (N spread, ricette a cavallo facciata→retro). Lo slider diventa lo strumento universale (testo+foto, N colonne anche irregolari) con default progressivi sull'avanzo; seed AI a richiesta; memoria per-libro della modalità. Ritira la delimitazione isolata dei `Block*`. | In sviluppo | **`SPEC_acquisizione.md`** (supera `SPEC_multipagina.md`, archiviato) · Memoria · post-lancio. Brainstorming fondatore + Recipees, 2026-06-05. Implementa la sessione Memoria, branch `feat/acquisizione-motore-unico` (repo Heirloom). **Stato 2026-06-06:** motore/editor completato (slider universali, default progressivi §5.1, crop foto unificato, seed AI on-demand, architettura sequenza-fogli con Singola/Doppia funzionanti) — 8 commit, build verde, review indipendenti. **Differita** la *convergenza* (instradare le 3 modalità dall'acquisizione verso l'editor unico + ritiro Block* + multipagina cross-foglio): tocca 3 flussi esistenti (pipeline pagina-singola `isSinglePageManual`, editor per-pagina, Block*) → richiede decisioni di prodotto + verifica a vista. Piano + checklist device in `docs/superpowers/plans/2026-06-05-acquisizione-motore-unico.md`. |
| Acquisizione multilingua | Oggi la cattura on-device è **hardcoded sull'italiano** (verificato nel codice 2026-05-30): voce/audio via `SpeechTranscriber` con `locale = "it-IT"` (chiamanti `ClaudeService.swift:107`, `PlanIntakeView.swift:313`); OCR `recognitionLanguages = ["it-IT","en-US"]`; il video eredita entrambi. La strutturazione AI (Claude) è già language-agnostic — il collo di bottiglia è la cattura, non l'AI. Test del fondatore in **bulgaro**: il parlato non veniva catturato. **Due tier:** (a) **lingue supportate da Apple** (francese, tedesco, spagnolo, russo…) → quick-win: passare un `locale` scelto/auto invece di hardcodare it-IT, sia per `SFSpeechRecognizer` sia per `recognitionLanguages` di Vision; (b) **lingue NON supportate da Apple** (bulgaro e altri alfabeti/locale fuori dal set Apple) → serve un'altra strada: STT cloud (es. Whisper) per la voce, e per l'OCR foto la via **Claude-vision** (manda la foto direttamente a Claude, che fa OCR+strutturazione in un passo, qualunque alfabeto), bypassando Apple Vision. | Idea | Memoria · post-lancio. Cross-prodotto: il percorso Claude-vision è condiviso col concept «Import nativo in Domus da foto» (area Domus). Da scrivere una mini-spec prima dello sviluppo: scelta lingua (auto da contenuto vs scelta utente), quali lingue Apple-native abilitare subito, e dove instradare le lingue fuori-set. |
...
| Ricerca — pulizia della UX | Snellire la schermata di ricerca. (a) **Rimuovere la sezione "suggerimenti"** — presente solo sul web, giudicata poco utile. (b) Tag e categorie oggi appaiono tutti insieme appena si apre la ricerca, formando un elenco lunghissimo (sia web sia mobile): renderli **espandibili / collassabili** con un piccolo bottone. | Fatto | Domus · commit `c4249c8`. (a) Sezione Suggerimenti rimossa. (b) Tag collassabili con bottone ▼/▲ nell'intestazione «Filtra per tag», default chiusi; tag attivi visibili anche da chiuso per permettere la deselezione; stato persistito in `localStorage`. Layout unico responsivo — funziona identicamente su desktop e mobile. Confermato a vista dal fondatore. |
| Ricerca — Filtra per ricettario | In /search aggiungere una sezione «Filtra per ricettario» analoga a «Filtra per tag»: chip collassabili, default espansi (al contrario dei tag), filtro OR su `cookbookId`. Mostra propri ricettari + condivisi. | Fatto | Domus · commit `dc64864`. Logica identica ai tag (localStorage, active pills visibili da chiuso). Default aperto (inverso dei tag). Filtro OR: una ricetta passa se il suo `cookbookId` è tra i ricettari selezionati. Confermato a vista dal fondatore. |
| Fix: tag/categorie e ricettari sparsi sulla home mobile | Nella home mobile (/recipes) appaiono chip tag/categorie E chip ricettari — duplicati rispetto alla sidebar desktop, inutili e confusivi. Vanno rimossi: i filtri per tag e ricettario stanno solo in /search. | Fatto | Domus · commit `dc64864` (tag + categorie) + `d55ceb8` (ricettari). Rimossi `CategoryFilters`, `toggleCategory`, `clearCategories`, `categoryStats`, il blocco `lg:hidden` ricettari e `toggleCookbook` orfana. La home è ora identica su mobile e desktop — filtri solo in /search (mobile) e sidebar (desktop). Confermato a vista dal fondatore. |
...
| Dispensa / inventario frigo (+ funzioni smart) | Tenere l'**inventario di cosa si ha in casa** — dispensa e frigorifero. È il *punto di partenza* per una famiglia di funzioni smart, da costruire a strati: **(1) Inventario base** — l'utente registra/aggiorna i prodotti che ha (quantità, eventuale scadenza). **(2) Ricette da ciò che hai** — cercare ricette che usano i prodotti in dispensa: prima **nel proprio ricettario**, poi anche **sul web / siti specializzati**; l'utente può lasciar scegliere all'app in base a ciò che sa esserci, o selezionare lui i prodotti da usare. Una ricetta trovata sul web si **importa con un click** nel ricettario e si può "cucinare stasera". **(3) Riconciliazione dispensa ↔ lista della spesa** — quando la lista nasce dalle ricette scelte, confrontarla con la scorta: es. servono 22 uova, ne hai 5 in frigo → l'app mostra la scorta attuale e l'utente decide se ridurre la quantità da comprare o no. Cucinare una ricetta può scalare la dispensa. | Idea | Domus · post-lancio. Feature a strati: l'inventario base abilita tutto il resto. **Cross-prodotto / dipendenze:** lo strato (2) sul ricettario riusa la ricerca per ingredienti (lega al concept «nome canonico ingrediente» per il match affidabile); lo strato (2) sul web riusa lo scraping/import esistente + l'import con un click; lo strato (3) si innesta sulla lista della spesa aggregata (`addMenuToShoppingList`, normalizzazione). Modello dati nuovo (collection dispensa per utente, additivo). Da scrivere una spec dedicata: granularità prodotti, scadenze, come l'AI matcha dispensa↔ingredienti, scaling al consumo. |
| Lista della spesa standalone (non solo da ricette) | Una lista della spesa **svincolata dalle ricette**: poter aggiungere voci in modo estemporaneo (a mano) e usare Domus come una **vera app di gestione lista spesa**, non solo come destinazione di ciò che esce dalle ricette/menù. Convive con la lista generata dalle ricette (oggi `addMenuToShoppingList` + vista aggregata): le voci libere si mescolano o restano in una sezione propria, da decidere. **Da valutare lo scope:** solo voci attinenti al cibo, oppure estendere a qualunque categoria (detersivi, casa…) ispirandosi alle app/siti specializzati di lista spesa. | Idea | Domus · post-lancio. Estende la lista spesa esistente (`/shopping`): oggi gli item nascono dalle ricette/menù; qui si aggiunge l'inserimento manuale libero. Si lega alla Dispensa (riga sopra) e all'export verso app di spesa online (riga sotto). Decisione di scope (solo-cibo vs generale) prima della spec. |
